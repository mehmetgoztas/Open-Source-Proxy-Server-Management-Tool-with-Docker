...
public class ServerHandler extends Thread {
    ...
    public ServerHandler(Socket clienSocket) throws IOException {
        ...
        log = new Helper(new File(Paths.get("ManusDei", "proxy", "proxylog.log").toAbsolutePath().toString()));
    }

    @Override
    public void run() {
        try{    
            byte[] headerArr = new byte[6969];
            int hc = 0;
            ...
            if (header.contains("BAN-URL ")) {
                Helper writeFile = new Helper(new File(Paths.get("ManusDei", "proxy", "ban.txt").toAbsolutePath().toString()));
                String s = header.substring(header.indexOf(" "), header.indexOf("\r\n"));
                URL url = new URL(s);
                writeFile.appendToFile(url.getHost());
                this.clientSocket.close();
                log.appendToFile("LOG: " + this.clientSocket.getInetAddress().toString() + " " + LocalDate.now() + " " + header);
                return;
            }

            if(header.contains("PROXYLOG")) {
                log.appendToFile("LOG: " + this.clientSocket.getInetAddress().toString() + " " + LocalDate.now() + " " + header);
                String s = log.readFromFile();
                dos.writeBytes(s);
                this.clientSocket.close();
                return;
            }

            log.appendToFile("LOG: " + this.clientSocket.getInetAddress().toString() + " " + LocalDate.now() + " " + header.replace("\n", " ").replace("\r", " "));
            ...
            if(methodName.equals("GET") || methodName.equals("HEAD")
                || methodName.equals("POST") || methodName.equals("OPTIONS")) {
                ...
                Helper readFile = new Helper(new File(Paths.get("ManusDei", "proxy", "ban.txt").toAbsolutePath().toString()));
                String readFromFile = readFile.readFromFile();
        ...
        if (statusCode == 200) {
            ...
            if (size <= 314572800) {
                ...
                if (responseHeader.contains("Last-Modified: ")) {
                    ...
                    ProxyServer.getCache().put(domain + shortPath, new MemoryCache(p, size, date));
                }
            }
            else {
